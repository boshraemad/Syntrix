...
import {
  ...
  Globe, Network, Activity, Users, AlertTriangle, Boxes, Lock, Wifi, Cable, Power,
} from 'lucide-react';
...
export default function HostDetail() {
  ...
  const openAlerts = host.alerts.filter((a) => a.status === 'OPEN' || a.status === 'IN_PROGRESS').length;
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-2xl font-bold tracking-tight">{host.host.name}</h1>
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="flex flex-col gap-1.5 items-start">
              <Badge color={sevColor(host.meta.criticality)}>{host.meta.criticality} asset</Badge>
        ...
        {activeTab === 'Overview' && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard label="Risk score" value={host.risk.score} color={sevColor(host.risk.level)} />
              <StatCard label="Open alerts" value={openAlerts} color={openAlerts ? '#ef4444' : '#22c55e'} />
              <StatCard label="Services" value={host.services.length} color="#38bdf8" />
              <StatCard label="Vulnerabilities" value={host.posture.vulnerabilities.length} color={host.posture.vulnerabilities.length ? '#f97316' : '#22c55e'} />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Section title="Identity & Inventory" icon={Boxes}>
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Hostname" value={host.host.name} />
                  <Field label="FQDN" value={host.host.hostname} />
                  <Field label="Domain" value={host.host.domain} />
                  <Field label="Host ID" value={host.host.id} />
              ...
              <Section title="Hardware" icon={Cpu}>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Field label="Memory" value={`${host.hardware.memoryGb} GB`} />
                  <Field label="Disk" value={`${host.hardware.diskFreeGb} GB free / ${host.hardware.diskGb} GB`} />
                </div>
              </Section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Section title="Network Summary" icon={Globe}>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                  <Field label="IP addresses" value={host.host.ip.join(', ')} />
        ...
        {activeTab === 'Security' && (
          <>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Section title="Security Posture" icon={Shield}>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Field label="Compliance score" value={host.posture.compliance.score === null ? 'n/a' : `${host.posture.compliance.score}%`} />
                </div>
              </Section>
            ...
            <Section title="Vulnerabilities" icon={Lock}>
              {host.posture.vulnerabilities.length === 0 ? (
