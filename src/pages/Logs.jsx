...
import { Plus, ChevronDown, Pencil, Loader2 } from 'lucide-react';
import useLogSources from '@/features/logsources/hooks/useLogSources';
...
export default function Logs() {
  ...
  const { data: logSources, loading, error } = useLogSources({ page: 1, limit: 10 });

  const handleSaved = (saved) => {
    const wasEdit = modal?.mode === 'edit';
    setModal(null);
    // React Query بيمسح الكاش تلقائياً من خلال الـ Mutations، مش محتاجين تريجر هنا
    showSuccessToast(wasEdit ? 'Log source updated' : `Log source "${saved?.name || 'source'}" added`);
  };
    ...
    <div className="pt-6 pb-8 px-4 text-white flex-1 h-full font-sans w-full flex flex-col">
      ...
      <div className="flex-1 bg-transparent mt-2 overflow-x-auto">
        {loading ? (
          ...
          <div className="text-center py-10 text-red-400 font-mono text-sm">
            Error: {error}
          </div>
        ) : (
            ...
            <tbody>
              {logSources.map((doc) => (
                ...
                >
                  ...
                  <td className="py-3 px-4 text-gray-400">{doc.source?.description}</td>
                  <td className="py-3 px-4 text-right font-mono text-gray-300">
                    {doc.ingestion?.totalEvents?.toLocaleString() || 0}
                  </td>
                  ...
                  <td className="py-3 px-4 text-right align-middle">
                    <button
                      onClick={() => setModal({ mode: 'edit', source: doc })}
